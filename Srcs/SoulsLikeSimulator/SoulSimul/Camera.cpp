...
// 스페이스-쉽 카메라는 객체에 물리적으로 고정
// 그러므로 카메라의 이동과 회전은 플레이어 객체의 이동과 회전과 일치
...
/*1인칭 카메라의 y - 축과 z - 축의 회전은 플레이어를 기준으로 회전을 하지만
x - 축으로는 카메라를 기준으로 회전(사람은 목을 회전하는 것과 유사)을 할 수 있다.*/
CFirstPersonCamera::CFirstPersonCamera(CCamera* pCamera) : CCamera(pCamera)
{
	...
	if (pCamera)
	{
		...
		이것은 스페이스-쉽 카메라의 로컬 y-축 벡터가 어떤 방향이든지 1인칭 카메라(대부분 사람인 경우)
		의 로컬 y축 벡터가 월드좌표의 y-축이 되도록 즉, 똑바로 서있는 형태로 설정한다는 의미이다. 
...
CThirdPersonCamera::CThirdPersonCamera(CCamera* pCamera) : CCamera(pCamera)
{
	...
	이것은 스페이스-쉽 카메라의 로컬 y-축 벡터가 어떤 방향이든지 3인칭 카메라(대부분 사람인 경우)의 
