...
void CObjectsShader::BuildObjects(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList, CScene* pScene, void* pContext)
{
	...
	for (int i = 0; i < TEXTURES; i++) pScene->CreateShaderResourceViews(pd3dDevice, ppTextures[i], 0, 3);
	...
	for (int i = 0, x = -xObjects; x <= xObjects; x++)
	{
		for (int y = -yObjects; y <= yObjects; y++)
		{
			for (int z = -zObjects; z <= zObjects; z++)
			{
				...
				D3D12_GPU_DESCRIPTOR_HANDLE d3dCbvGPUDescriptorHandle = pScene->CreateConstantBufferView(pd3dDevice, d3dGpuVirtualAddress + (((sizeof(CB_GAMEOBJECT_INFO) + 255) & ~255) * i), ((sizeof(CB_GAMEOBJECT_INFO) + 255) & ~255));
				pRotatingObject->SetCbvGPUDescriptorHandle(d3dCbvGPUDescriptorHandle);
