...
CGameFramework::CGameFramework()
{
	...
	m_UIManager = new UIManager(); // UIManager 객체 생성
}

CGameFramework::~CGameFramework()
{
	if (m_UIManager) delete m_UIManager; // UIManager 객체 삭제
}
...
bool CGameFramework::OnCreate(HINSTANCE hInstance, HWND hMainWnd)
{
	...
	// UIManager 초기화
	if (!m_UIManager->Initialize(hMainWnd, m_pd3dDevice, m_pd3dSrvDescriptorHeap)) {
		MessageBox(0, L"Failed to initialize UIManager.", 0, 0);
		exit(1);
	}
...
void CGameFramework::CreateRtvAndDsvDescriptorHeaps()
{
	...
	srvHeapDesc.NumDescriptors = 10; // 원하는 수 만큼
	srvHeapDesc.Type = D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV;
	srvHeapDesc.Flags = D3D12_DESCRIPTOR_HEAP_FLAG_SHADER_VISIBLE;

	HRESULT hr = m_pd3dDevice->CreateDescriptorHeap(&srvHeapDesc, IID_PPV_ARGS(&m_pd3dSrvDescriptorHeap));
	if (FAILED(hr)) {
		MessageBox(0, L"SRV Descriptor Heap 생성 실패", L"Error", MB_OK | MB_ICONERROR);
		exit(1);
	}
}
...
extern IMGUI_IMPL_API LRESULT ImGui_ImplWin32_WndProcHandler(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam); // 추가해야함
...
void CGameFramework::ProcessInput()
{
	...
	마우스 왼쪽 또는 오른쪽 버튼이 눌러질 때의 메시지(WM_LBUTTONDOWN, WM_RBUTTONDOWN)를 
	처리할 때 마우스를 캡쳐하였다. 
...
void CGameFramework::FrameAdvance()
{
	...
	switch (m_pScene->GetCurrentSceneState())
	{
	case SceneState::MAIN_MENU:
		m_UIManager->MainMenuUI();
		break;
	...
	case SceneState::MULTI_SETTING:
		m_UIManager->MultiSettingUI();
		break;
	case SceneState::MULTI_PLAY:
		m_UIManager->MultiPlayUI();
		break;
	default:
		break;
	}
	...
	프리젠트가 끝나면 렌더 타겟 버퍼의 상태는 프리젠트 상태(D3D12_RESOURCE_STATE_PRESENT)
