...
void CGameFramework::ProcessInput()
{
	...
	DWORD dwDirection = 0;
	/* 키보드의 상태 정보를 반환한다. 화살표 키(‘→’, ‘←’, ‘↑’, ‘↓’)를 누르면 플레이어를 오른쪽/왼쪽(로컬 x-축), 
	앞 / 뒤(로컬 z-축)로 이동한다. ‘Page Up’과 ‘Page Down’ 키를 누르면 플레이어를 위/아래(로컬 y-축)로 이동한다. */
	...
	if ((dwDirection != 0) || (cxDelta != 0.0f) || (cyDelta != 0.0f))
	{
		...
		// ### 이동 속력을 임의로 지정
		if (dwDirection) m_pPlayer->Move(dwDirection, 4.0f, true);
	}
	...
	cyDelta)
{
	...
	{
		/*
		if (dwDirection & DIR_FORWARD) m_pSelectedObject->MoveForward(+5.0f);
		if (dwDirection & DIR_BACKWARD) m_pSelectedObject->MoveForward(-5.0f);
		if (dwDirection & DIR_LEFT) m_pSelectedObject->MoveStrafe(+5.0f);
		if (dwDirection & DIR_RIGHT) m_pSelectedObject->MoveStrafe(-5.0f);
		if (dwDirection & DIR_UP) m_pSelectedObject->MoveUp(+5.0f);
		if (dwDirection & DIR_DOWN) m_pSelectedObject->MoveUp(-5.0f);
		*/
	}
